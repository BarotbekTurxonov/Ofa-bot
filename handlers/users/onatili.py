...
async def chemistry(call: types.CallbackQuery):
    ...
    await EnrollMother.full_name.set()
    await call.message.delete()

@dp.message_handler(state=EnrollMother.full_name)
async def step1(msg: types.Message, state : FSMContext):
    ...
    if full_name == '/cancel':
        ...
        await EnrollMother.next()




@dp.message_handler(state=EnrollMother.phone)
async def step1(msg: types.Message, state : FSMContext):
    phone = msg.text
    ...
    🔗 Tanlagan Fan : <tg-spoiler> <u><b>Ona tili</b></u></tg-spoiler>"
...
async def step1(msg: types.Message, state : FSMContext):
    ...
    if full_name == '/cancel':
        ...
        await EnrollMotherEng.next()




@dp.message_handler(state=EnrollMotherEng.phone)
async def step1(msg: types.Message, state : FSMContext):
    phone = msg.text
    if phone == '/cancel':
        await msg.answer('Action canceled!', reply_markup=boshlangich_button_eng)
    ...
    ✅ Telegram : @{msg.from_user.username}\n\n\
    🔗 Selected Subject : <tg-spoiler> <u><b>Mother Tongue</b></u></tg-spoiler>"
