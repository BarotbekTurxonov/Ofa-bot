...
async def chemistry(call: types.CallbackQuery):
    ...
    await EnrollFizics.full_name.set()
    await call.message.delete()

@dp.message_handler(state=EnrollFizics.full_name)
async def step1(msg: types.Message, state : FSMContext):
    ...
    if full_name == '/cancel':
        ...
        await EnrollFizics.next()



@dp.message_handler(state=EnrollFizics.phone)
async def step1(msg: types.Message, state : FSMContext):
    phone = msg.text
    ...
    🔗 Tanlagan Fan : <tg-spoiler> <u><b>Fizika</b></u></tg-spoiler>"
...
async def step1(msg: types.Message, state : FSMContext):
    ...
    if full_name == '/cancel':
        ...
        await EnrollFizicsEng.next()




@dp.message_handler(state=EnrollFizicsEng.phone)
async def step1(msg: types.Message, state : FSMContext):
    phone = msg.text
    ...
    ✅ Telegram : @{msg.from_user.username}\n\n\
    🔗 Selected Subject : <tg-spoiler> <u><b>Physics</b></u></tg-spoiler>"
